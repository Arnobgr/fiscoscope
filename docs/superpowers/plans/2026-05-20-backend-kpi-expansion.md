...
### Task C2: Build the Tax Expenditure KPI from GTED *(BUILT 2026-05-20, user-approved)*

> **Done.** Implemented same day after user go-ahead. Build notes + the unit-mismatch gotcha are in the CLAUDE.md Session C entry. **Correction to the C1 note:** the GTED Zenodo *concept* record id is **`5940165`** (concept DOI `10.5281/zenodo.5940165`); `12585656` is the v1.3.0 *version* record, not the concept. The fetcher resolves `…/records/5940165/versions/latest`.

**Source (verified in the C1 spike):** GTED v1.3.2, Zenodo record `17312217` (concept `5940165`), `GTED_FullDatabase_20251010.xlsx`, sheet `RevenueForgone`, `Country=="FRA"`. CC-BY-4.0 → attribution required.
...
- [x] **Step 1: Write `fetchers/tax_expenditure.py`**
...
- [x] **Step 2: Implement `compute_tax_expenditure()`** (France-only — PRD names no peer source)
...
- [x] **Step 3: Register in the pipeline** — add the fetcher to `run_annual()` and flip `kpi_tax_expenditure` from the `NotImplementedError`/`skipped` branch to a live `_run_step`.

- [x] **Step 4: Verify + commit** — module `__main__` + `python run_pipeline.py --mode annual --no-upload`; confirm `kpi_tax_expenditure` is now `ok`. Update the CLAUDE.md Session C entry's decision line (BUILD → done) and commit.
